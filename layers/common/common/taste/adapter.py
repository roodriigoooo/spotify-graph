...
def build_profile(
    ...
    # ── artist facet ──
    # Preference order: a precomputed v2 footprint written by the ingest job, then
    # recency-weighting raw events here, then the v1 snapshot.
    if record.get("artistWeights"):
        artist_weights = {str(k): _f(v) for k, v in record["artistWeights"].items()}
    elif events:
